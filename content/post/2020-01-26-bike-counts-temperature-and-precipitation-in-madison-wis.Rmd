...
![Table of figures for non-zero count thresholds](/non-zero-thresholds.png)
...
*Edit 2020-02-07*
After publishing this post, I got [great feedback](https://github.com/vgXhc/datenecke/issues/1) on how to better handle the run length encoding and then getting back to the runs in the original dataset. This relies on the `rleid` function from the `data.table` package. Much more elegant! Thanks, [Jean-Marie Pivette](https://github.com/JMPivette)! 
...
counts2 %>% 
  ...
  summarise(start_date = first(Count_Date),
            duration = n(),
            Count = first(Count)) %>% 
  filter(duration  >= 25 & Count == 0)
```

JP also has good suggestions on additional data quality checks. I won't implement these for now but it's a good reference.

```{r echo=FALSE}
blogdown::shortcode('tweet', '1222289124889169922')
```
